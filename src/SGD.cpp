...
NumericVector cal_recharge_c(NumericVector t,       // Simulation day
                             NumericVector R,       // daily precipitation (mm)
                             NumericVector E0,      // daily potential evaporation calculated with Penman monteith method (mm)
                             NumericVector H2O_SB1, // depth of water in SB1 (mm)
                             NumericVector H2O_SB2, // depth of water in SB2 (mm)
...
)
{
  ...
  NumericVector H2O3_AQ(GWL.size(), NA_REAL);
  ...
  NumericVector dxT(GWL.size(), 0.0);
  NumericVector xT3(GWL.size(), NA_REAL);
  NumericVector SWvol(GWL.size(), NA_REAL);
  NumericVector FWLdrop(GWL.size(), NA_REAL);

  List aq = as<List>(params["aq"]);
  List con = as<List>(consts["consts"]);

  double aq_Sy = as<double>(aq["Sy"]);
  double aq_K = as<double>(aq["K"]);
  double con_x = as<double>(con["x"]);
  double aq_rho_s = as<double>(aq["rho_s"]);
  double aq_rho_f = as<double>(aq["rho_f"]);
  double con_W = as<double>(con["W"]);
  double aq_z0 = as<double>(aq["z0"]);
  double aq_a = as<double>(aq["a"]);
  double con_Area = as<double>(con["Area"]);
  double con_xT = as<double>(con["xT"]);
  double con_dxT = as<double>(con["dxT"]);
  ...
  for (int i = 0; i < GWL.size(); ++i)
  {
    ...
    if (WrechgAve[i] > 1E-30)
    {
      ...
      xn1[i] = SGD1[i] / (WrechgAve[i] / 1000) / con_W;
      xn2[i] = SGD2[i] / (WrechgAve[i] / 1000) / con_W;
      hn1[i] = sqrt(std::pow(SGD1[i] / con_W, 2) / (WrechgAve[i] / 1000) / aq_K + aq_rho_s / aq_rho_f * aq_z0 * aq_z0) - aq_z0;
      hn2[i] = sqrt(std::pow(SGD2[i] / con_W, 2) / (WrechgAve[i] / 1000) / aq_K + aq_rho_s / aq_rho_f * aq_z0 * aq_z0) - aq_z0;
      ...
      if (M1[i] < 1)
      {
        xT1[i] = (SGD1[i] / con_W) / (WrechgAve[i] / 1000) - sqrt(pow((SGD1[i] / con_W) / (WrechgAve[i] / 1000), 2) - aq_K * aq_a * (1 + aq_a) * (aq_z0 * aq_z0) / (WrechgAve[i] / 1000));
      }
      if (M2[i] < 1)
      {
        xT2[i] = (SGD2[i] / con_W) / (WrechgAve[i] / 1000) - sqrt(pow((SGD2[i] / con_W) / (WrechgAve[i] / 1000), 2) - aq_K * aq_a * (1 + aq_a) * (aq_z0 * aq_z0) / (WrechgAve[i] / 1000));
      }
    }
    else
    {
      SGD1[i] = (aq_K / 2 / con_x * aq_rho_s / (aq_rho_s - aq_rho_f) * std::pow(GWL[i], 2)) * con_W;
      SGD2[i] = (aq_K * ((std::pow((GWL[i] + aq_z0), 2) - aq_rho_s / aq_rho_f * std::pow(aq_z0, 2))) / 2 / con_x * con_W);
      xn1[i] = 1000000000;
      xn2[i] = 1000000000;
      hn1[i] = 1000;
      hn2[i] = 1000;
      ...
      if (M1[i] < 1)
      {
        xT1[i] = aq_K * aq_a * (1 + aq_a) * aq_z0 * aq_z0 / 2 / (SGD1[i] / con_W);
      }
      if (M2[i] < 1)
      {
        xT2[i] = aq_K * aq_a * (1 + aq_a) * aq_z0 * aq_z0 / 2 / (SGD2[i] / con_W);
      }
    }
    ...
    SGDdrop[i] = SGD[i] / con_Area * 1000 / aq_Sy;
    PumpingDrop[i] = Pumping[i] / con_Area * 1000 / aq_Sy;
    ...
    else
    {
      ...
      else
      {
        if (xT1[i] >= xT3[i - 1])
        {
          dxT[i] = std::min(con_dxT, xT1[i] - xT3[i - 1]);
        }
        else
        {
          dxT[i] = std::max(-con_dxT, xT1[i] - xT3[i - 1]);
        }
      }
      xT3[i] = xT3[i - 1] + dxT[i];
    }
    SWvol[i] = aq_Sy * aq_z0 * xT3[i] / 3;
  ...
  DataFrame result = DataFrame::create(Named("H2O1_AQ") = H2O1_AQ,
                                       ...
                                       Named("xT3") = xT3,
                                       Named("SWvol") = SWvol,
                                       Named("FWLdrop") = FWLdrop,
                                       Named("H2O3_AQ") = H2O3_AQ
