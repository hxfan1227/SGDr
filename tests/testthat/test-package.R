...
test_that("calibratable parameters are correctly loaded", {
  ...
  expect_contains(names(x), c('bucket1', 'bucket2', 'cn', 'aquifer'))
  expect_contains(names(x$bucket1), c('layer', 'z', 'rho_b', 'm', 'rho_p', 'Ks', 'n', 'pE'))
  expect_contains(names(x$bucket2), c('layer', 'z', 'rho_b', 'm', 'rho_p', 'Ks', 'n', 'pZ'))
  expect_contains(names(x$cn), c('CN2', 'pF'))
  expect_contains(names(x$aquifer), c('Td', 'rho_s', 'rho_f', 'Ka', 'z0', 'Sy', 'XT', 'dxT_max'))
})

test_that("constant parameters are correctly loaded", {
  x <- json_to_parameter_list(test_path('testdata', 'consts.json'))
  expect_contains(names(x), c('x', 'W', 'A', 'L', 'WC'))
  expect_type(x$WC, 'list')
})

test_that("estimate_sgd() checks parameters", {
  inputData <- read.csv(test_path('testdata', 'test_data.csv'))
  expect_snapshot_error(estimate_sgd(inputData, constParams = c(1, 2, 3), calibratableParams = c(1, 2, 3), nw = 180, warmup = 1e5))
  expect_snapshot_error(estimate_sgd(inputData, constParams = c(1, 2, 3), calibratableParams = c(1, 2, 3), nw = 180, warmup = 120))
})
...
test_that("estimate_sgd_from_pars() returns same reults as estimate_sgd()", {
  ...
  x <- estimate_sgd(inputData, calibratableParamsList, constParamsList, warmup = 50, nw = 180)
...
test_that('estimate_sgd_from_pars() checks parameter names', {
  ...
  constParamsList <- json_to_parameter_list(test_path('testdata', 'consts.json'))
  parNames <- names(calibratableParams)[which(!(names(calibratableParams) %in% c('bucket1.layer', 'bucket2.layer')))]
  expect_snapshot_error(estimate_sgd_from_pars(pars = c(calibratableParams[parNames], nw = 180),
                                      parset = parNames,
                                      parnames = parNames,
                                      inputDf = inputData,
                                      calibratableParams = calibratableParamsList,
...
test_that('change_unknown_pumping() works', {
  ...
  inputData[, q1 := NULL]
  preferred_q1 <- 30
  new_input <- change_unknown_pumping(x = preferred_q1, input_df = inputData, pumping_df = pumping_data, yearly_df = yearly_pumping_df)
  ...
  x <- estimate_sgd(input_from_file, calibratableParamsList, constParamsList, warmup = 1500, nw = 180)
  y <- estimate_sgd_from_pars(pars = c(calibratableParams[parNames],
                                       nw = 180,
                                       pumping = preferred_q1),
                              parset = parNames,
