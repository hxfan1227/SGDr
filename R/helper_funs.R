...
#' @param obs_data A data.frame of daily observed groundwater level data. See Details.
#' @param start_date A valid string that can be transformed into data format using \code{\link[lubridate]{ymd}}.
#' @return A \code{\link[ggplot2]{ggplot}} object.
#' @details
#' The \code{obs_data} is a data.frame containing the following 2 columns:
#'  * \code{date}   Observation date
#'  * \code{wl}     Daily observed groundwater level (m) 
#' @export

model_diagnose <- function(result, obs_data, start_date) {
    plot_df <- result %>% 
...
#' @param type A character string indicating the type of plot. Can be one of 'pred', 'obs', 'comp'.
